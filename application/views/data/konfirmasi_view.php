...
<html lang="en">
    <head>
        ...
        <script type="text/javascript">
            error = '<?php echo $error; ?>';
                    ...
                    site_url = '<?php echo site_url(); ?>/data/konfirmasi/';
            ...
            var app = angular.module('main', ['ngTable', 'ngTableExport','picardy.fontawesome']).
                    ...
                    var data = [];
                            ...
                            $('#img_load').show();
                            $http.get('<?php echo site_url('data/konfirmasi/order_pembelian_show'); ?>')
							...
							$scope.custom_filter = function () {
								...
								$http.post('<?php echo site_url('data/konfirmasi/order_pembelian_show'); ?>', {"filter_tanggal":temp})
									.success(function($order) {
			...
			function update_status(index)
            {
                    $.ajax({
                    type: "POST",
                            url: "<?php echo site_url('data/konfirmasi/order_pembelian_show_by_id') ?>",
                            timeout: 20000,
                            data:
            ...
            $('.loading_gear_gif').show();
                    $.ajax({
                    type: "POST",
                            url: "<?php echo site_url('data/konfirmasi/delete') ?>",
                            timeout: 20000,
                            data:
                            'datamodel=' + $(index).attr("datamodel")
                            ...
                            alertify.success('Hapus sukses');
                                    location.reload();
                                    window.location.replace("<?php echo site_url('data/konfirmasi/'); ?>");
                            }
                            else if (result == "2")
                            {
    ...
    <body ng-app="main">
                            ...
                            <div class="col-md-12">
                                ...
                                <table ng-table="tableParams"  class="table ng-table-responsive latar" export-csv="csv">
                                    <tr ng-repeat="(datamodel, order_pembelian) in $data | filter: filtering| filter: filterings|  filter:order_pembelian.order_pembelian_no as results"  ng-class-odd="'Bg_genap'" ng-class-even="'Bg_ganjil'"  qtip=""  datamodel={{order_pembelian.datamodel}}  ng-class="{ 'emphasis': order_pembelian.order_pembelian_no == '<?php echo $this->session->userdata("order_pembelian_no") ?>'}">
										...
										<td align="center" class="col-md-2 borderkanan" data-title="'No Order'" sortable="'order_no'">
                                            <span><b>{{order_pembelian.order_no}}</b></span>
                                        </td> 
										 <td class="col-md-3 borderkanan" data-title="'Customer'" sortable="'customer_nama'">
                                            <span>{{order_pembelian.customer_nama}}</span>
                                        </td> 
                                           <td align="right" class="col-md-2 borderkanan" data-title="'Jumlah'" sortable="'jml'">
                                            <span>{{order_pembelian.jml|currency:"Rp "}}</span>
                                        </td> 
                                           <td align="right" class="col-md-3 borderkanan" data-title="'Rekening'" sortable="'order_berat'">
                                            <span><b>[{{order_pembelian.bank}}</b>] <br/>{{order_pembelian.no_rek}} a.n. {{order_pembelian.nama_rek}}</span>
                                        </td> 
                                        <td align="center" class="col-md-1 borderkanan" data-title="'Status'" sortable="'status'"> 
                                            <span >{{order_pembelian.status}}</span>  
                                        </td>
        ...
        <?php $this->load->view("data/konfirmasi_crud_view"); // remodal  ?>
