...
class data_user_customer extends CI_Model {
    ...
    public function update_last_login($user_id) {
        ...
        $this->db->update('data_user_customer', $data);
    }
	...
	{  
		$temp=$this->db->insert('data_user_customer', $data); 
	...
	{  
		...
		$this->db->delete('data_user_customer', $data);
	...
	{
		$data_lama=$this->db->query('select * from data_user_customer where user_id="'.$user_id.'"')->num_rows();
		
	   $data = array(
			'is_delete' => '1',
		); 
        $this->db->where('user_id', $user_id);
       $temp= $this->db->update('data_user_customer', $data); 
